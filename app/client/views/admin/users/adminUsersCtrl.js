...
angular.module('reg').controller('AdminUsersCtrl', [
  ...
  function ($scope, $state, $stateParams, UserService) {
    ...
    UserService.getPage($stateParams.page, $stateParams.size, $stateParams.query).then((response) => {
      updatePage(response.data);
    });

    $scope.$watch('queryText', function (queryText) {
      UserService.getPage($stateParams.page, $stateParams.size, queryText).then((response) => {
        updatePage(response.data);
      });
    });
    ...
    $scope.acceptUser = function ($event, user, index) {
      ...
      swal({
        ...
        swal({
          ...
          dangerMode: true,
          title: 'Are you sure?',
          text: 'Your account will be logged as having accepted this user. ' + 'Remember, this power is a privilege.',
          icon: 'warning',
        }).then((value) => {
          if (!value) {
            return;
